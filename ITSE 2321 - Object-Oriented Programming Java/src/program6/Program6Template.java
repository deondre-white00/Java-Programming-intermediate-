...
public class Program6Template {
	...
	double newAverPay;
	   ...
	   public void calculate()
	   {
		   ...
		   output.format("%s: %,.2f%n", "Employee Salary", salary);
		   output.format("%s: %,.2f%n", "Employee Raise", raise);
		   totalRaises += raise;
		   output.format("%s: %,.2f%n","Company Raise", totalRaises);
		   count++;
		   averRaise = totalRaises/count;
		   output.format("%s: %,.2f%n","Average Raise", averRaise);
		   totalPayroll += salary;
		   output.format("%s: %,.2f%n","Total Payroll", totalPayroll);
		   totalAverPay = totalPayroll/count;
		   output.format("%s: %,.2f%n","Total Average Pay", totalAverPay);
		   newPayroll = totalPayroll + totalRaises;
		   output.format("%s: %,.2f%n","New Payroll", newPayroll);
		   newAverPay = newPayroll/count;
		   output.format("%s: %,.2f%n","New Average Payroll", newAverPay);	
		   output.format("%n%n");
	   }
